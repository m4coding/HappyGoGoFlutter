...
/// 协调器
class CustomScrollCoordinator {
  /// 页面主 CustomScrollView 控制
  final String pageLabel = "page";

  CustomScrollController _pageScrollController;
  double Function() pinnedHeaderSliverHeightBuilder;

  CustomScrollPosition get _pageScrollPosition => _pageScrollController.position;
  ...
  /// 获取主页面滑动控制器
  CustomScrollController pageScrollController([double initialOffset = 0.0]) {
    assert(initialOffset != null, initialOffset >= 0.0);
    _pageInitialOffset = initialOffset;
    _pageScrollController = CustomScrollController(this,
        debugLabel: pageLabel, initialScrollOffset: initialOffset);
    return _pageScrollController;
  }

  /// 创建并获取一个子滑动控制器
  CustomScrollController newChildScrollController([String debugLabel]) =>
      CustomScrollController(this, debugLabel: debugLabel);
  ...
  void updateUserScrollDirection(
      CustomScrollPosition position, ScrollDirection value) {
