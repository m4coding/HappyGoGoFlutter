...
///弹窗相关
///refrence：https://github.com/YYFlutter/flutter-custom-dialog
class HDialog {
  //================================弹窗属性======================================
  ...
  bool canBackDismissible = true; //是否响应返回键消失
  ...
  bool fitKeyBoard = false; //是否适配键盘--底部弹窗有输入框时不会被键盘遮挡
  ...
  HDialog build([BuildContext ctx]) {
        ...
        child: Row(
          ...
          children: <Widget>[
            Expanded(
              child: FlatButton(
                onPressed: () {
                  if (isClickAutoDismiss) { //ontap回调如果放在dismiss后面，当执行跳转到新的page时会出现fucoseNode找不到的问题。。
                    dismiss();
                  }
  ...
  void show([x, y]) {
    ...
    CustomDialog(
      ...
      duration: duration,
      child: getChild(mainAxisAlignment, crossAxisAlignment),
    );
  }

  Widget getChild(MainAxisAlignment mainAxisAlignment, CrossAxisAlignment crossAxisAlignment) {
    ...
    List<Widget> list = [];
    if (fitKeyBoard) {
      list.add(Expanded(child: GestureDetector(child: Container(color: Colors.transparent,), onTap: (){
    ...
    Widget willPopScope = WillPopScope(
      onWillPop: () async {return canBackDismissible;},
      child: Padding(
        padding: margin,
        child: Column(
...
class CustomDialog {
  ...
  bool _barrierDismissible; //点击蒙版背景，是否dismiss
  Gravity _gravity;
